...
char* check_trend(SensorData sensor_data, SensorMetrics metrics) {
    ...
    float avg_accel[metrics.acceleration_size / WINDOW_SIZE];
    int avg_index = 0;

    // calculate average of acceleration of window size 10 points
    for (int i = 0; i <= metrics.acceleration_size - WINDOW_SIZE; i += WINDOW_SIZE) {
        float sum = 0.0;
        for (int j = 0; j < WINDOW_SIZE; j++) {
            sum += sensor_data.acceleration[i + j];
        }
        avg_accel[avg_index++] = sum / WINDOW_SIZE;
    }
    ...
    for (int i = 1; i < avg_index; i++) {
        if (avg_accel[i] > avg_accel[i - 1]) {
            is_decreasing = 0; // 如果有上升，说明不是下降趋势
        } else if (avg_accel[i] < avg_accel[i - 1]) {
            is_increasing = 0; // 如果有下降，说明不是上升趋势
        }
    }

    // if there is no trend
    if (!is_increasing && !is_decreasing) {
        current_trend_state = NO_TREND; 
        return "No trend detected"; // 
    }

    // if temp variation percentage is above 30%
    if (temp_diff_percentage > 30) {
        current_trend_state = TEMPERATURE_TREND;
        return "Trend detected due to temperature variation";
    }

    // check if it's sensor activation
    if (sensor_data.is_active) {
        current_trend_state = ACTIVATION_TREND;
        return "Trend detected due to sensor activation";
    }

    // return trend result
    if (is_increasing) {
        current_trend_state = INCREASING_TREND;
        return "Trend detected: Acceleration is consistently increasing";
    } else if (is_decreasing) {
        current_trend_state = DECREASING_TREND;
        return "Trend detected: Acceleration is consistently decreasing";
    }
...
// Function to correct acceleration data using the overall slope
void correct_acceleration_with_slope(SensorData *sensor_data, SensorMetrics *metrics) {
    for (int i = 0; i < metrics->acceleration_size; i++) {
        // Apply slope correction to acceleration data
        sensor_data->acceleration[i] -= metrics->slope;
    }
}
